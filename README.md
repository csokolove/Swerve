# NOTE: This swerve code is based on [Team 364's BaseFalconSwerve](https://github.com/Team364/BaseFalconSwerve), however has been adapted for use with a NavX and SRX Mag Encoders.
**This code is currently untested, however once it has been tested, this notice will be removed.**

# *A fork of* BaseFalconSwerve </br>

**Basic Swerve Code for a Swerve Module using Falcon Motors, a CTRE SRX Mag Encoder, and a Kauai Labs NavX2** </br>
...
1. Gyro Settings: ```navxId``` and ```invertGyro``` (ensure that the gyro rotation is CCW+ (Counter Clockwise Positive)
2. ```chosenModule```: 
...
8. ```srxMagEncoder``` and ```angleMotorInvert```: Both must be set such that they are CCW+. <br><b>If you are using a supported module, this value will be automatically set.</b>
...
10. ```Module Specific Constants```: set the Can Id's of the motors and SRX Mag Encoders for the respective modules, see the next step for setting offsets.
11. Setting Offsets
    ...
    * Open smartdashboard (or shuffleboard and go to the smartdashboard tab), you will see 4 printouts called "Mod 0 SRX Mag Encoder", "Mod 1 SRX Mag Encoder", etc. 
